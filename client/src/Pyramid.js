...
const WordDetailModal = ({ word, words, setWords, onClose }) => {
    const [generatedExample, setGeneratedExample] = useState(null);
    const [jobStatus, setJobStatus] = useState(null); // null, 'pending', 'processing', 'failed'
    ...
    useEffect(() => {
        ...
        const exampleSubscription = channel
    ...
    const handleCreateJob = async () => {
        ...
        setGenerationError(null);
        setGeneratedExample(null); // 이전 예문 지우기
    ...
    const handleToggleComplete = async () => {
        const newStatus = currentWord.status === 'completed' ? 'learned' : 'completed';
        const updatedWord = { ...currentWord, status: newStatus };
        setCurrentWord(updatedWord);
        setWords(prevWords => prevWords.map(w => w.id === currentWord.id ? updatedWord : w));
        await supabase.from('user_word_progress').update({ status: newStatus }).eq('word_id', currentWord.id);
    };

    const card = toFSRSCard(currentWord, currentWord);
        ...
        >
            <div
                style={{
                    ...
                    minWidth: 260, maxWidth: 360, color: 'white',
            ...
            >
                <span
                    style={{
                        position: 'absolute', top: 18, right: 22, fontSize: 26,
                        color: currentWord.status === 'completed' ? '#30D158' : '#888',
                        cursor: 'pointer', zIndex: 2
                    }}
                    title={currentWord.status === 'completed' ? '완료 취소' : '완료로 표시'}
                ...
                {currentWord.english_meaning && (
                    <div style={{ fontSize: 16, color: '#FFD60A', marginBottom: 8 }}>
                        영어 뜻: {currentWord.english_meaning}
                    </div>
                )}
                <div style={{ fontSize: 13, color: '#aaa', marginBottom: 4 }}>빈도수: #{currentWord.frequency_rank}</div>
                
                <div style={{
                    margin: '12px 0 8px 0', fontSize: 15, fontWeight: 700, color: color,
                    background: color + '22', borderRadius: 8, padding: '6px 0',
                ...
                {currentWord.status && (
                    <div style={{ fontSize: 13, color: '#FFA726', marginBottom: 8 }}>학습상태: {currentWord.status}</div>
                )}

                <div style={{
                    margin: '16px 0 8px 0', padding: '12px 10px',
...
// --- Pyramid Component (No changes below this line) ---
    ...
    return (
        <div style={{ 
            ...
            <div style={{
                ...
                alignItems: 'center', padding: `8px ${isMobile ? '16px' : '24px'}`,
                boxSizing: 'border-box', zIndex: 1200, background: '#1a1a1a',
                borderBottom: '1px solid #333', height: isMobile ? 'auto' : 56, minHeight: 56
            }}>
                <div style={{
                    display: 'flex', justifyContent: 'space-between', alignItems: 'center',
                    width: '100%', marginBottom: isMobile ? '8px' : 0,
                }}>
                    {viewMode === 'layer' && (
                        <button 
                            onClick={() => { setViewMode('all'); setSelectedLayer(null); }}
                            style={{
                                ...buttonStyle, marginLeft: 0, color: '#4FC3F7',
                    ...
                    <span style={{ 
                        fontSize: isMobile ? '24px' : '32px', fontWeight: 700, color: 'white', 
                        marginRight: 'auto', marginLeft: viewMode === 'layer' ? (isMobile ? '12px' : 0) : 0,
                    }}>
            ...
            {viewMode === 'layer' && (
                <div style={{
                    position: 'fixed', top: 56, left: 0, width: '100%',
                    display: 'flex', justifyContent: 'center', gap: '16px',
                    padding: '12px 0', zIndex: 1200, background: '#1a1a1a',
                    borderBottom: '1px solid #333'
                }}>
                    <button onClick={() => setSortMode('pos')} style={{ ...buttonStyle, background: sortMode === 'pos' ? '#4FC3F7' : '#2c2c2e', color: sortMode === 'pos' ? '#1a1a1a' : 'white' }}>품사별 그룹</button>
                    <button onClick={() => setSortMode('freq')} style={{ ...buttonStyle, background: sortMode === 'freq' ? '#4FC3F7' : '#2c2c2e', color: sortMode === 'freq' ? '#1a1a1a' : 'white' }}>빈도수 정렬</button>
                </div>
            )}

            {/* 스크롤 가능한 컨테이너 */}
            <div style={{
                position: 'fixed', top: viewMode === 'layer' ? 112 : (isMobile ? 100 : 56),
